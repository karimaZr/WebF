...
const Dashboard = ({ id }) => {
    ...
    const fetchGroups = async () => {
        setLoading(true);
        const rep = await axios.get(`${url}/groupe/professor/${id}`);
        setGroups(rep.data);
        setLoading(false);
    };
    ...
    const fetchStudents = async () => {
        setLoadings(true);
        const rep = await axios.get(`${url}/student/professor/${id}`);
                            ...
                            students.map((student, index) => (
                                <div onClick={() => handleStudentClick(student.id)} className='flex flex-col  items-center rounded-t-xl shadow-xl bg-white hover:scale-105'>
                                    <div className='bg-blue-400 bg-opacity-80  rounded-t-xl w-full flex justify-center '>
                                        <img src={student.photo === null ? "https://mdbcdn.b-cdn.net/img/Photos/new-templates/bootstrap-chat/ava3.webp" : `data:image/png;base64, ${student.photo}`} className='w-48 h-48 rounded-full p-3' />
                                    </div>
